...
export class MarkettogglecellComponent implements OnInit {
  ...
  UpdateMarketStatus(){
    ...
    this.SportSettingdata.UpdateMktStatus(this.data.id,this.data.isActive).subscribe(data=>{
  ...
  UpdateMktbetStatus(){
    ...
    this.SportSettingdata.UpdateMktStatus(this.data.id,this.data.isAllowBet).subscribe(data=>{
      if (data.status == "Success") {
        if (this.isActive) {
          this.isActive = false;
        }
